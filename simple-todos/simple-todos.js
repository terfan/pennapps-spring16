...
if (Meteor.isServer) {
    ...
    Meteor.methods({
        getTweetHTTP: function (text) {
            ...
            T.get('search/tweets', { q: text, count: 5}, Meteor.bindEnvironment(function(err, data, response) {
              var results = data["statuses"];
              for (var i = 0; i < results.length; i++) {
                console.log("tweet: "+ results[i].text);
                url = "http://gateway-a.watsonplatform.net/calls/text/TextGetTextSentiment?apikey=0e2d5d3c35387b178bd6f8d27c2e44ca83fbc5aa&outputMode=json&text=" + results[i].text;

                var score = Meteor.http.call("GET", url).data["docSentiment"]["score"]
                  //Tweets.insert({text: results[i].text});
                  Tweets.insert({text: results[i].text + " " + score});
              }
